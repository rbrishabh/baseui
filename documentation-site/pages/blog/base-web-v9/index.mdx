...
For more on how our theme system works, including how to override default theme values, check out [this page](/theming/custom-themes).
...
You can access any of Uber's brand colors on the `colors` object exported by `baseui/tokens`.
...
We have adjusted the default typography values included in our theme. The scale has shifted upwards, so for instance, `font200` is now `14px` rather than `12px`. We have updated all of the components in the library to use lower typography values such that sizing is unchanged.
...
<Caption>
  We are deprecating font460 and font470. These properties may be removed in a
...
If you want to stay in line with `baseui`'s default typography ramp, we recommend the following mapping, which we used ourselves when updating all of our components:
...
Note, you _could_ update the variables to their nearest equivalent size in the new scale but this is essentially opting out of the semantics of the design system. It will likely lead to frustration with future changes to `baseui`. If you need to maintain exact sizing in your project, consider adding your values to the theme object and using those in your source code instead.
...
3. **`Caption1` and `Caption2` have been deprecated** in favor of `Paragraph4` and `Label4` respectively. The `Caption` components have not been removed but instead alias `Paragraph` and `Label` components. A future major may see these components removed, so it is best to swap your use of `Caption` for `Paragraph` and `Label`.
...
We consider this to be one of the main features of Base Web. Folks who use the Base design system, as Uber does on hundreds of projects, know that by staying on the latest version of `baseui`, their app remains on-brand and consistent with other products.
